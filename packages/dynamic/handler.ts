...
export class DynamicHandler {
    ...
    toPropsData(data,configProps:DynamicConfigProps){
        ...
        const cloneData=utils.cloneObj(data)
    ...
    jsonValueType(val) {
        ...
        if (currType === 'object') {
            const currDataType = this.dataTypes.find(ele => utils.isSameObject(val, ele.defaultValue))
            if (currDataType) { return currDataType }
        }
        const currDataType = this.dataTypes.find(ele => utils.isSameObject(val, ele.defaultValue) || ele.type.toLowerCase() === currType || (ele.type === 'Bool' && currType === 'boolean'))
        return currDataType
    }
    jsonToConfig(jsonData) {
        ...
        for (let key in jsonData) {
            if (key) {
                ...
                importData.push({
                    "keyID": "key_" + utils.randomNumber().toString(),
                    "keyName": key,
                    "keyCode": key,
                    "children": currData,
    ...
    initConfig(children) {
        children.forEach((ele) => {
            ...
            if (!ele.keyID) {
                ele.keyID = "key_" + utils.randomNumber().toString()
            }
        })
    }
    returnConfig(data) {
    ...
    recoverArrayData(item, valueData: any = null) {
        ...
        if (valueData) {
            ...
            valueData.forEach((ele) => {
                let itemData: any = utils.cloneObj(item["arrayObjData"])
            ...
            if (item.config.arrayConfig.arrayDefaultLength === undefined || item.config.arrayConfig.arrayDefaultLength === '') {
                defaultArrayData.push(utils.cloneObj(item["arrayObjData"]))
            } else {
    ...
    configResult(config, showKeyField = 'keyCode') {
        const currConfig = utils.cloneObj(config)
