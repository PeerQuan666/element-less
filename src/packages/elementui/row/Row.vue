...
function getSpan() {
    if (slots.default) {
        if (slots.default().length == 1 && slots.default()[0].type.toString() == 'Symbol(Fragment)') {
            let cols = (slots.default()[0].children as any).filter(ele => ele.type && ele.type.name == 'ElsCol')
            colData.value.count = cols.length
            return 24 / cols.length;
        }
        let cols = slots.default().filter(ele => ele.type && (ele.type as any).name == 'ElsCol')
        colData.value.count = cols.length
        return 24 / cols.length;
    }
    return 24
