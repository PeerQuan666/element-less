...
<template>
    ...
    <div class="condition-node" :class="{'is-error':!isValidate}" v-if="item.data.StepType == 'When'" @click="attrDrawVisible=true">
    ...
    <els-drawer v-model="attrDrawVisible" :title="item.data.Name" :initBody="true" class="work-flow-drawer" size="40%" @close="closeAttrSetting" :show-close="false" append-to-body>
        <div >
            <els-caption type="left">步骤配置</els-caption>
            <els-form v-model="item.data" label-width="130px" v-model:isValidate="formValidate" ref="itemForm">
                ...
                <component :is="dynamicName" v-model="item.data.Inputs" ref="dynamicTag" v-model:isValidate="comValidate" :config="item.data.InputControl"
                    :return-obj="true" :config-data="currConfig.InputControl"></component>
          
                <els-caption type="left">Outputs</els-caption>
                <els-form-item label="步骤输出字段" v-if="currConfig.OutputFields" tip="用step.字段名取值">
