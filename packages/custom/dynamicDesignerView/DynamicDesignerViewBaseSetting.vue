...
<template>
    <els-form v-model="currSelectItem" labelPosition="top">
        ...
        <els-select label="数据类型" 
            ...
            @click-option="currSelectItem.defaultValue='';"
        ...
        <template v-if="currSelectItem.componentGroup==='Form'">
            <els-radio-button label="默认值" :resetValueByChangeData="false" v-model="currSelectItem.defaultValue" v-if="currSelectItem.componentTypeName==='Switch'&&baseConfig['active-value']!==undefined">
                <els-option :value="baseConfig['inactive-value']">{{baseConfig['inactive-text']||baseConfig['inactive-value']}}</els-option>
                <els-option :value="baseConfig['active-value']">{{baseConfig['active-text']||baseConfig['active-value']}}</els-option>
            </els-radio-button>
            <els-select label="默认值" :resetValueByChangeData="false" v-model="currSelectItem.defaultValue" :multiple="currSelectItem.componentTypeName==='Checkbox'" v-else-if="['Select','Radio','Checkbox'].includes(currSelectItem.componentTypeName)&&baseConfig['data']!==undefined" v-bind="baseConfig">
            </els-select>

            <els-textarea label="默认值" prop="defaultValue" :rows="3" v-else-if="currSelectItem.dataTypeName==='String'"  ></els-textarea>
            <els-input-number label="默认值" prop="defaultValue" v-else-if="currSelectItem.dataTypeName==='Number'" controls-position="right"  ></els-input-number>
